...
class CH_InitialConditions(UserExpression):
    ...
    def eval(self, values, x):
        Cn = self.Cn
        r = sqrt((x[0] * x[0]) / 0.8 + ((x[1]) * (x[1])) / 1.2)
...
class Problem:
    ...
    def refine_mesh(self, nr_refinements):
        mesh = self.nonrefinedmesh
        for i in range(nr_refinements):
            phi_value, mu_value = self.CHsol.split()
            ...
            for c in cells(mesh):
                ...
                phi_limit = 0.98
                if (abs(phi_value(r_value, z_value))) < (phi_limit):
                    cell_markers[c] = True
                else:
        ...
        boundaries = MeshFunction('size_t', mesh, mesh.topology().dim() - 1,
                                      value=0)
        botwall_firstpart = CompiledSubDomain('near(x[0], 5.0) && on_boundary')
        z_axis = CompiledSubDomain('near(x[0], 0.0) && on_boundary')
        firstwall = CompiledSubDomain('near(x[1], -5.0) && on_boundary')
        secondwall = CompiledSubDomain('near(x[1], 5.0) && on_boundary')
    ...
    def formulate_problem(self, Dt):
        ...
        bcu_secondwall = DirichletBC(self.S.sub(0), Expression(inflow_profile, degree=2),
                                    self.boundaries, 1)
        bcp_firstwall = DirichletBC(self.S.sub(1), Constant((4*0)), self.boundaries, 1)
...
def SCHsolve(nr=50, nz=100, Pe=3e-3, Cn=0.06, Ca=1e0, theta1=0, theta2=0, lamb=1.0,
    ...
    nonrefinedmesh = RectangleMesh(Point(0.0, -5.0), Point(5.0, 5.0), nr, nz)
...
if __name__ == "__main__":
    ...
    folder = 'Cn=' + str(Cn) + \
        'Pe=' + str(Pe) + 'nr' + str(nr) + \
        'Ca=' + str(Ca) + 'lamb=' + str(lamb) + 'theta1=' + str(theta1) + \
        'theta2=' + str(theta2) + 'Dt=' + str(Dt) + '/'
    file_name = 'no_slip/' + folder
    ...
    #File(folder + 'Ssol_setu_alpha' + str(alpha) + '.xml') << Ssol
    #File(folder + 'CHsol_setu_alpha' + str(alpha) + '.xml') << CHsol
